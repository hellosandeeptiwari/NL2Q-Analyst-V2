...
function SettingsPanel({ isOpen, onClose, onDatabaseChange }: SettingsPanelProps) {
  ...
  const testConnection = async () => {
    ...
    try {
      // Fix: Send the correct format that the backend expects
      const requestData = {
        db_type: dbConfig.type,
        host: dbConfig.host,
        port: dbConfig.port,
        database: dbConfig.database,
        user: dbConfig.username,  // backend expects 'user', not 'username'
        password: dbConfig.password,
        // Snowflake-specific fields
        account: dbConfig.account,
        warehouse: dbConfig.warehouse,
        schema: dbConfig.schema,
        role: dbConfig.role || ''  // ensure role is never undefined
      };
      ...
      if (response.data.success) {
        ...
        setStatusMessage(response.data.message || response.data.error || 'Connection failed');
      }
    } catch (error: any) {
      ...
      if (error.response) {
        ...
        errorMessage = 'Cannot reach backend server. Please ensure the backend is running on port 8000.';
      } else {
        // Other error
